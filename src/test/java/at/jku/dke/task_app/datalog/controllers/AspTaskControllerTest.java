...
class AspTaskControllerTest {
    ...
    void initDb() {
        ...
        this.taskId = this.repository.save(new AspTask(1L, BigDecimal.TEN, TaskStatus.APPROVED, group, "human(X) :- person(X).", 2)).getId();
    }
    ...
    void getShouldReturnOk() {
        given()
            ...
            // WHEN
            .when()
            .get("/api/task/asp/{id}", this.taskId)
            // THEN
            .then()
    ...
    void getShouldReturnNotFound() {
        given()
            ...
            // WHEN
            .when()
            .get("/api/task/asp/{id}", this.taskId + 1)
            // THEN
            .then()
            .log().ifValidationFails()
            .statusCode(404);
    ...
    void createShouldReturnCreated() {
        given()
            ...
            .header(AuthConstants.AUTH_TOKEN_HEADER_NAME, ClientSetupExtension.CRUD_API_KEY)
            .contentType(ContentType.JSON)
            .body(new ModifyTaskDto<>(this.taskGroupId, BigDecimal.TWO, "asp", TaskStatus.APPROVED,
                new ModifyDatalogTaskDto("human2(X) :- person(X).", "human2(X)?;person(X)?", "person(mike).")))
            // WHEN
            .when()
            .post("/api/task/asp/{id}", this.taskId + 2)
            // THEN
            .then()
            .log().ifValidationFails()
            .statusCode(201)
            .contentType(ContentType.JSON)
            .header("Location", containsString("/api/task/asp/" + (this.taskId + 2)))
    ...
    void updateShouldReturnOk() {
        given()
            ...
            // WHEN
            .when()
            .put("/api/task/asp/{id}", this.taskId)
            // THEN
            .then()
            .log().ifValidationFails()
            .statusCode(200)
    ...
    void updateShouldReturnNotFound() {
        given()
            ...
            // WHEN
            .when()
            .put("/api/task/asp/{id}", this.taskId + 1)
            // THEN
            .then()
            .log().ifValidationFails()
            .statusCode(404);
    ...
    void deleteShouldReturnNoContent() {
        given()
            ...
            // WHEN
            .when()
            .delete("/api/task/asp/{id}", this.taskId)
            // THEN
            .then()
            .log().ifValidationFails()
            .statusCode(204);
    ...
    void deleteShouldReturnNoContentOnNotFound() {
        given()
            ...
            // WHEN
            .when()
            .delete("/api/task/asp/{id}", this.taskId + 1)
            // THEN
            .then()
            .log().ifValidationFails()
            .statusCode(204);
    ...
    void mapToDto() {
        ...
        // Act
        var result = new DatalogTaskController(null).mapToDto(task);
