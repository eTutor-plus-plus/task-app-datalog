...
class DatalogSubmissionControllerTest {
    ...
    @Autowired
    private DatalogSubmissionRepository submissionRepository;
    ...
    void initDb() {
        ...
        var task = this.repository.save(new DatalogTask(1L, BigDecimal.TWO, TaskStatus.APPROVED, group, "human(X) :- person(X).", List.of("human(X)?"), null));
        this.taskId = task.getId();

        var submission = new DatalogSubmission("test-user", "test-id", task, "de", 3, SubmissionMode.SUBMIT, "human(X) :- person(X).");
        ...
        this.ungraded = this.submissionRepository.save(new DatalogSubmission("test-user", "test-id", task, "de", 3, SubmissionMode.SUBMIT, "human(X) :- person(X).")).getId();
    }
    ...
    void getReturnsOk() {
        given()
            ...
            // WHEN
            .when()
            .get("/api/submission/dlg/{id}/result", this.graded.toString())
            // THEN
            .then()
            .log().ifValidationFails()
            .statusCode(200)
    ...
    void getReturnsOkAfterWait() {
        ...
        given()
            ...
            // WHEN
            .when()
            .get("/api/submission/dlg/{id}/result", this.ungraded.toString())
            // THEN
            .then()
            .log().ifValidationFails()
            .statusCode(200)
    ...
    void getReturnsNotFound() {
        given()
            ...
            // WHEN
            .when()
            .get("/api/submission/dlg/{id}/result", UUID.randomUUID().toString())
            // THEN
            .then()
            .log().ifValidationFails()
            .statusCode(404);
